-- Apply Codewire active rows and tombstones only when they are newer than the
-- stored version. Writes are exposed only through this function so older or
-- custom clients cannot bypass the updated_at comparison with direct writes.
-- IDs originate on the device and can legitimately repeat when a backup is
...
alter table public.jobs
  add constraint jobs_pkey primary key (user_id, id);
...
grant execute on function public.sync_codewire(jsonb, jsonb)
  to authenticated;

-- The RPC owns all writes. Authenticated clients retain read access for the
-- pull phase, while cached pre-migration clients fail safely instead of doing
-- an unconditional table upsert.
